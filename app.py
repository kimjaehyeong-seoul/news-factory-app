...
def install_and_import_soynlp():
    """런타임에 soynlp 설치 및 import"""
    try:
        # soynlp 설치 시도
        subprocess.check_call([sys.executable, "-m", "pip", "install", "soynlp>=1.0.0"])
        
        # 설치 후 import
        from soynlp.word import WordExtractor
        from soynlp.noun import LRNounExtractor_v2
        
        st.success("✅ soynlp 고급 한국어 처리 엔진 활성화")
        return True, WordExtractor(), LRNounExtractor_v2()
    except Exception as e:
        st.warning(f"⚠️ soynlp 설치 실패: {e}")
...
def safe_import():
    ...
    # soynlp 런타임 설치 및 로드
    success, word_extractor, noun_extractor = install_and_import_soynlp()
