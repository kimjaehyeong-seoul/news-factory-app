...
try:
    from google_sheets_utils_deploy import (
        load_data_from_google_sheets_cached, preprocess_dataframe,
...
def main():
    ...
    # 데이터 로드 (캐시 기능 포함)
    with st.spinner("🔄 데이터를 불러오는 중... (캐시 확인 중)"):
        df = load_data_from_google_sheets_cached()
    ...
    # ==========================================
    # 백업파일 탭 구조 복원 (konlpy 기반)
    # ==========================================
    
    # 메인 탭 구성 (백업파일과 동일하게)
    tab1, tab2, tab3 = st.tabs(["📰 기사 분석", "🔍 AI 인사이트", "📊 상세 분석"])
    
    # ==========================================
    # Tab 1: 기사 분석 (백업파일 기능 복원)
    # ==========================================
    with tab1:
        st.header("📰 뉴스 기사 분석")
        ...
        with col1:
            st.metric("📊 총 기사 수", len(filtered_df), delta=f"+{len(filtered_df) - 100}")
        
        with col2:
            if "전체가중치" in filtered_df.columns and not filtered_df["전체가중치"].empty:
                avg_weight = filtered_df["전체가중치"].mean()
                st.metric("⚖️ 평균 가중치", f"{avg_weight:.3f}")
            else:
                st.metric("⚖️ 평균 가중치", "미계산")
        ...
        # 가중치 분포 히스토그램 (st.histogram 오류 해결)
        st.subheader("📈 가중치 분포")
        if "전체가중치" in filtered_df.columns and not filtered_df["전체가중치"].empty:
            # 히스토그램 데이터 생성
            hist_data = filtered_df["전체가중치"].dropna()
            if len(hist_data) > 0:
                # 구간별 데이터 생성
                hist_df = pd.DataFrame({
                    '가중치': hist_data
                })
                st.bar_chart(hist_df['가중치'])
            else:
                st.warning("가중치 데이터가 없습니다.")
        else:
            st.warning("가중치 데이터가 없습니다.")
        ...
        # 키워드 분석 (konlpy Okt 기반)
        st.subheader("🔍 주요 키워드 (konlpy 기반)")
        
        with st.spinner("🔄 키워드 추출 중..."):
            titles = filtered_df["제목"].dropna().tolist()
            keywords = extract_keywords_okt(titles, 15)
            ...
            if keywords:
                ...
                with col1:
                    safe_display_dataframe(keyword_df, "키워드 분석")
    ...
    # ==========================================
    # Tab 2: AI 인사이트 (백업파일 기능 복원)
    # ==========================================
    with tab2:
        st.header("🤖 AI 인사이트 분석")
        
        # 오늘의 기획 아이템 (백업파일과 동일)
        st.subheader("💡 오늘의 AI 기획 아이템")
        ...
        # 월별/주차별 인사이트 (백업파일과 동일)
        st.subheader("📅 월별/주차별 AI 인사이트")
    ...
    with tab3:
        ...
        if selected_industry != "전체":
            ...
            if industry_keywords:
                ...
                safe_display_dataframe(industry_keyword_df, f"{selected_industry} 키워드")
        ...
        selected_columns = st.multiselect(
            "표시할 컬럼 선택",
            all_columns,
            default=['제목', '매체', '업계', '보도날짜', '전체가중치'][:5]
        )
        
        if selected_columns:
            display_df = filtered_df[selected_columns].head(50)
            safe_display_dataframe(display_df, "선택된 데이터")
        ...
        st.download_button(
            label="📥 데이터 CSV 다운로드",
            data=csv,
            file_name=f'뉴스팩토리_분석_{selected_industry}_{datetime.now().strftime("%Y%m%d")}.csv',
            mime='text/csv'
        )
    ...
    <div style='text-align: center; padding: 1rem; margin-top: 2rem; 
        ...
        <p>✨ 백업파일 기능 완전 복원 + 구글 시트 캐싱 시스템</p>
    </div>
    """, unsafe_allow_html=True)
