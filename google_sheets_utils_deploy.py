"""
Google Sheets 유틸리티 모듈 - soynlp 기반 완전 기능 버전 (네이버 링크 하이퍼링크 포함)
"""
...
def extract_company_names_soynlp(title, content=""):
    """soynlp를 사용한 고급 기업명 추출"""
    try:
        ...
        # 간단한 학습 (빠른 처리를 위해)
        word_extractor.train([full_text])
        noun_extractor.train([full_text])
        ...
        for word, score in words.items():
            if (len(word) >= 2 and len(word) <= 8 and
                score.cohesion_forward >= 0.05 and
                re.search(r'[가-힣]', word) and
                not word.isdigit()):
                companies.add(word)
...
def map_industry_advanced(row, company_to_industry):
    ...
    # soynlp 기반 기업명 추출
    if SOYNLP_AVAILABLE:
        companies = extract_company_names_soynlp(title, content)
    else:
        companies = extract_company_names_simple(title, content)
...
print(f"📊 soynlp 사용 가능: {SOYNLP_AVAILABLE}")

# 버전 정보
__version__ = "3.2.0"
__author__ = "뉴스팩토리 개발팀"
__description__ = "soynlp 기반 한국어 뉴스 분석 + 가중치 개선·AI 월·주차 리뷰"
